...
- **可在 Claude Code 每轮对话结束时弹出 IDE 提示**（基于 Claude Code 的 Stop hook，提供一键安装命令）
...
```bash
cd claude-ref-ext
npx --yes @vscode/vsce package          # 生成 claude-ref-sender-1.4.0.vsix
code --install-extension claude-ref-sender-1.4.0.vsix
```
...
此后 Claude 每答完一轮，IDE 右下角就会弹出提示（文案可用 `claudeRef.turnEndMessage` 自定义）。
...
| `claudeRef.notifyOnTurnEnd` | boolean | `false` | 在 Claude Code 每轮对话结束时弹出 IDE 提示。依赖 Stop hook，需先执行命令「Claude Ref: 安装对话结束提示 Hook」（详见下文） |

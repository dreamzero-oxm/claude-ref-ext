...
- **可在 IDE 内逐块 Review Claude 的改动**：每轮改完后用内置 diff 呈现「改动前（红）/ 改动后（绿）」，逐块「接受（保留）/ 拒绝（回退原代码）」，或对当前文件「接受全部 / 拒绝全部」未确认改动，配 `Alt+A/D/Z`、`Alt+Shift+A/D` 快捷键与底部 ↑/↓、「下一个文件」导航（基于一键安装的 Review Hook）
- **在 VSCode 内置 Git diff 视图中支持「转到定义」「查看引用」等符号跳转**：diff 里的原始/历史版本本是只读虚拟文档，语言服务默认不对其生效；现桥接到磁盘上的真实文件，使 F12、右键「转到定义 / 查看引用」、Ctrl/Cmd+Click 及悬停提示在 diff 视图内也能正常工作
...
### 在 Git diff 中转到定义 / 查看引用

VSCode 内置 Git diff 视图（源代码管理面板中点开一个改动文件时看到的左右对比）里，**原始版本 / 历史版本那一侧是只读虚拟文档**，VSCode 的语言服务只对磁盘上的真实文件生效，所以直接在 diff 里对符号按 `F12` 通常没有反应。

本扩展会把 diff 视图里的导航请求桥接到磁盘上的真实文件再求解，因此在 diff 视图内也可以正常使用：

- **转到定义**（`F12` / 右键「转到定义」/ `Ctrl`/`Cmd` + 点击）
- **查看引用 / 转到引用**（`Shift+F12` / 右键「查看引用」）
- **转到类型定义**、**转到实现**
- **悬停提示**（鼠标悬停显示类型/文档）

跳转结果始终落在真实文件上（与在普通编辑器中的体验一致）。由于 diff 里的历史版本内容可能与磁盘当前内容存在差异，扩展会优先按原文本就近匹配对应行，找不到时退回到同一行列，尽量保证跳转准确。

不需要该功能时，可将 `claudeRef.enableDiffNavigation` 设为 `false` 关闭（无需重启，改动即时生效）。
...
| `claudeRef.showSelectionCodeLens` | boolean | `true` | 选中代码时在选区上方显示「💬 将选中引用添加到终端」CodeLens 气泡。关闭后不再显示气泡，仍可用快捷键/右键菜单/命令面板发送 |
| `claudeRef.requireClaudeRunning` | boolean | `false` | 仅在当前活动终端正在运行 Claude Code 时才发送引用。开启后若当前活动终端没在跑 claude，则取消发送并提示，避免把 `@path` 误打进普通 shell。依赖 VSCode 终端 Shell 集成，不支持时降级为照常发送 |
...
| `claudeRef.enableDiffNavigation` | boolean | `true` | 在 VSCode 内置 Git diff 视图中启用「转到定义/查看引用/转到类型定义/转到实现/悬停」等符号导航。关闭后 diff 视图内不再桥接这些导航（不影响其他功能），改动即时生效 |
...
```json
{
  ...
  "claudeRef.showSelectionCodeLens": true,
  ...
  "claudeRef.enableDiffNavigation": true
...
3. 通过 `terminal.sendText()` 把文本写入当前活动终端（默认不追加换行）。

...
- **在 VSCode 内置 Git diff 视图中支持「转到定义 / 查看引用」等符号导航**：diff 视图里原始/历史版本一侧是只读虚拟文档（`git:` scheme），VSCode 语言服务默认只对磁盘上的真实文件生效，故此前在 diff 里对符号按 `F12` 没有反应。现为 `git:` scheme 注册桥接 Provider，把导航请求还原到磁盘上的真实文件后委派给语言服务求解，使「转到定义」（`F12`）、「查看引用 / 转到引用」（`Shift+F12`）、「转到类型定义」、「转到实现」与悬停提示在 diff 视图内也能正常工作。
  - 位置映射做了多级回退：优先按行号对齐，行内容不一致时在 ±200 行窗口内就近查找文本相同的行，仍未命中则退回原始行列，尽量保证跳转准确。
  - 新增配置项 `claudeRef.enableDiffNavigation`（默认 `true`），关闭后 diff 视图内不再桥接这些导航，改动即时生效、无需重启；不影响插件其他功能。
